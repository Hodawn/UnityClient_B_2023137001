...
public class ParabolicTrajectroy : MonoBehaviour
{
    ...
    public LineRenderer lineRenderer;       //Line Renderer 컴폰전트를 할당할 변수
    ...
    {
        float launchAngledRad = Mathf.Deg2Rad * launchAngle;                    // 발사 각도를 라디안으로 변환
        float launchDirectionRad = Mathf.Deg2Rad * launchDirection;

        //시간 t에서의 x,y,z 좌표 계산
        float x = launchPower * time * Mathf.Cos(launchAngledRad) * Mathf.Cos(launchDirectionRad);
        float z = launchPower * time * Mathf.Cos(launchAngledRad) * Mathf.Sin(launchDirectionRad);
        float y = launchPower * time * Mathf.Sin(launchAngledRad) + 0.5f * gravity * time * time;
